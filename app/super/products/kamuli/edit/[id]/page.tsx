...
export default function EditKamuliProductPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const resProd = await fetch(`/api/kamuli/products/${id}`);
        ...
        const resCat = await fetch("/api/kamuli/categories");
  ...
  const handleUpdateProduct = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const res = await fetch(`/api/kamuli/products/${id}`, {
        method: "PUT",
      ...
      if (res.ok) {
        ...
        router.push("/super/products/kamuli");
      } else {
